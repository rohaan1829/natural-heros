...
/** Pick a usable image URL from either Sanity asset data or a local
 *  fallback path. Returns null when neither is available. */
function imageSrc(
  sanity: SanityImage | undefined,
  fallback?: string,
): string | null {
  if (sanity?.asset) {
    try {
      return urlForImage(sanity).width(800).url();
    } catch {
...
/** Local gallery fallback when Sanity has no images yet. */
...
/** Local trust badge icon fallbacks, in display order. */
const fallbackBadgeIcons = [
  "/images/sub-badge-natural.svg",
  "/images/sub-badge-vegan.svg",
  "/images/sub-badge-plastic.svg",
  "/images/sub-badge-aluminium.svg",
];
...
export default function SubscribeClient({ data }: { data: SubscriptionProduct }) {
  // Resolve gallery sources: prefer Sanity images, fall back to locals.
  const galleryImages = (() => {
    if (data.galleryImages && data.galleryImages.length > 0) {
      const urls = data.galleryImages
  ...
  // First type option's id seeds the active state; falls back to "90day".
  const initialType = data.typeOptions?.[0]?.id ?? "90day";

  const [activeImage, setActiveImage] = useState(0);
  const [purchaseType, setPurchaseType] = useState<string>(initialType);
            ...
            >
              ...
              <div className="flex items-center gap-2 whitespace-nowrap border-y border-[#E2DFDF] py-2 font-sans text-[13px] leading-[140%] md:gap-[18px] md:text-[16px]">
                ...
                <div className="flex items-center gap-1 text-[#767473] md:gap-[7px]">
                  ...
                  <span>{data.reviews?.questions ?? 10} vragen</span>
                </div>
                <span className="h-4 w-px bg-[#767473] md:h-[19px]" aria-hidden />
                <span className="text-[#767473]">
                  {data.reviews?.users ?? "20m+ users"}
                </span>
              </div>
              ...
              <div className="flex flex-col gap-2 pt-3">
                <span className="font-sans text-[16px] font-semibold leading-[140%] text-ink">
                  {data.inhoudLabel ?? "1. Kies je inhoud"}
                </span>
                <button
                  type="button"
                  ...
                  <div className="flex items-center gap-2">
                    <div className="relative h-16 w-16 shrink-0">
                      <Image
                        src={
                          imageSrc(
                            data.inhoudOption?.image,
                            "/images/freshbalance.svg",
                          ) ?? "/images/freshbalance.svg"
                        }
                        alt=""
                        fill
                    ...
                    <span className="font-sans text-[18px] font-semibold leading-[140%] text-ink">
                      {data.inhoudOption?.label ?? "30 ml / Fresh Balance"}
                    </span>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className="font-sans text-[18px] font-semibold leading-[140%] text-ink">
                      {data.inhoudOption?.price ?? "€ 12.99"}
                    </span>
              ...
              {/* 2. Kies je type — N option cards driven by Sanity. */}
              <div className="flex flex-col gap-2 pt-3">
                <span className="font-sans text-[16px] font-semibold leading-[140%] text-ink">
                  {data.typeLabel ?? "2. Kies je type"}
                </span>
                ...
                {(data.typeOptions ?? []).map((opt) => (
                  <PurchaseOption
                    key={opt.id}
                    option={opt}
                    isActive={purchaseType === opt.id}
                    onSelect={() => setPurchaseType(opt.id)}
                    welcomeKitHeading={data.welcomeKitHeading}
                    welcomeKitItems={data.welcomeKitItems}
                  />
                ))}
              </div>
              ...
              {/* Trust badges — circles with icons + labels */}
                ...
                <div className="flex flex-row items-start gap-9 pt-3">
                  {data.trustBadges.map((badge, i) => (
                    <TrustBadge
                      key={i}
                      src={
                        imageSrc(badge.icon, fallbackBadgeIcons[i]) ??
                        fallbackBadgeIcons[i] ??
                        ""
                      }
                      label={badge.label}
                    />
              ...
              {data.trustList && data.trustList.length > 0 && (
                <ul className="flex flex-col gap-2 pt-2">
                  {data.trustList.map((item, i) => (
                    <li
                      key={i}
...
function PurchaseOption({
  ...
  option: SubscriptionTypeOption;
  isActive: boolean;
  onSelect: () => void;
  welcomeKitHeading?: string;
  welcomeKitItems?: WelcomeKitItem[];
}) {
  const { label, savings, bestValue } = option;
  // Only expand when the card has expanded-state content to show.
  const hasExpandedContent =
    Boolean(option.displayPrice) ||
    (option.inclusief && option.inclusief.length > 0) ||
    (welcomeKitItems && welcomeKitItems.length > 0);
                ...
                {welcomeKitItems.map((item, i) => (
                  ...
                  >
                    ...
                    <div className="relative h-20 w-20 shrink-0 md:h-24 md:w-24">
                      <Image
                        src={
                          imageSrc(item.image, "/images/sheabutter.svg") ??
                          "/images/sheabutter.svg"
                        }
                        alt={item.title}
                        fill
                        className="object-contain"
